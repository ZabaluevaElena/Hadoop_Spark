...
def load_dataset(path, spark):
    ...
    dataset = ratings.join(users, "user_id").join(movies, "movie_id")
    dataset = dataset.persist(pyspark.StorageLevel.MEMORY_ONLY)
    dataset = dataset.repartition(N_REPAR)
    return dataset
...
train = train.persist(pyspark.StorageLevel.MEMORY_ONLY)
test = test.persist(pyspark.StorageLevel.MEMORY_ONLY)
...
fm = learn_pipe.fit(train.repartition(N_REPAR))
